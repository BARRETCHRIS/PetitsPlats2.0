export default class Search {
    constructor(getDatas, displayRecipes) {
        ...
        // Ajoutz un gestionnaire d'événements à l'input main_search pour afficher/masquer la croix
        const mainSearchInput = document.getElementById('main_search');
    ...
    handleSearch(query) {
        ...
        if (query.length >= 3) {
            ...
            const matchingRecipes = this.searchRecipes(query);
    ...
    searchRecipes(query) {
        // Convertisse la requête en minuscules pour une correspondance insensible à la casse
        const lowerCaseQuery = query.toLowerCase();

        // Filtre les recettes qui correspondent à la requête dans les éléments name, ingredients et description
